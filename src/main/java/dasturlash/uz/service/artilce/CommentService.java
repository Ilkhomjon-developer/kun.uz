...
import dasturlash.uz.repository.article.CommentCustomRepository;
import dasturlash.uz.repository.article.CommentRepository;
...
public class CommentService {
    ...
    public Boolean delete(String id) {
        ...
        if (!SpringSecurityUtil.hasAnyRoles(ProfileRole.ROLE_ADMIN)) {
    ...
    public Page<CommentDTO> filter(CommentFilterDTO filter, int page, int size) {
        ...
        for (Object[] obj : filterResult.getContent()) {
            ...
            if (obj[6] != null) {
                ...
                reply.setId((Long) obj[6]);
                comment.setReply(reply);
            }

            ProfileDTO profile = new ProfileDTO();
            profile.setId((Long) obj[7]);
